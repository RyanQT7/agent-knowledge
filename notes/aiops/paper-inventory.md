...
| 2 | [FlowFixer — diagnosis-driven repair of agentic workflows](../../papers/aiops/flowfixer/notes.md) | [PDF](<../../sources/papers/AIOps_papers/Diagnosis-Driven_Automatic_Repair_for_Agentic_Workflow_via_Symbolic_Inference.pdf>) | Xuyan Ma et al. | 2026 / arXiv | Failure attribution, RCA, diagnosis, workflow repair | Agent traces, workflow/configuration, prompts, variables, tool interactions; multi-source | Symbolic execution traces/specifications; verification; repair patch generation | LLM: Yes; Agent: fixed iterative workflow-repair system, not live operational Agent | AgentFail/n8n workflow cases; no production infrastructure deployment | Medium | R0 | Batch 3; **Full Reading: Completed** |
...
| 4 | [Cloud Intelligence/AIOps 2.0 — knowledge-anchored agentic AIOps](../../papers/aiops/cloud-intelligence/notes.md) | [PDF](<../../sources/papers/AIOps_papers/FSE26-Cloud Intelligence_AIOps 2.0- Knowledge-Anchored Agentic AIOps.pdf>) | Dongmei Zhang et al. | 2026 / FSE Companion | Operational knowledge, incident management, RCA/remediation architecture | Logs, metrics, incidents, docs/TSGs, dependencies, operational signals; multi-source | Operational Knowledge Artifacts; anchored retrieval; bounded execution and escalation | LLM: proposed execution layer; Agent: proposed architecture, not standalone validated system | Operational motivation; standalone production validation and scale unclear | High | R0 | Batch 3; **Full Reading: Completed** |
| 5 | [Cloud-OpsBench — deterministic Agentic SRE evaluation](../../papers/aiops/cloud-opsbench/notes.md) | [PDF](<../../sources/papers/AIOps_papers/FSE26-Freezing the Crime Scene- A State Snapshot Paradigm for Reproducible Agentic SRE Evaluation.pdf>) | Guangba Yu, Yilun Wang, Michael R. Lyu | 2026 / FSE Companion | Agentic SRE process and RCA-workflow evaluation | Metrics, logs, Kubernetes configuration, runtime state, mocked interfaces; multi-source operational state | Deterministic state snapshots / digital twins; process-oriented benchmark proposal | LLM: target systems; Agent: evaluation target, not a deployed system | Prototype/testbed; knowledge-driven injected incidents; scale/release status unclear | High | R0 | Batch 3; **Full Reading: Completed** |
...
| 10 | [CHIEF — hierarchical failure attribution for LLM-based MAS](../../papers/aiops/chief/notes.md) | [PDF](<../../sources/papers/AIOps_papers/From_flat_logs_to_causal_graphs_Hierarchical_failure_attribution_for_llm-based_multi-agent_systems.pdf>) | Yawen Wang et al. | 2026 / arXiv | Failure attribution and RCA for multi-agent systems | MAS logs/trajectories, thoughts, actions, observations, tool results, inter-agent messages; multi-source trace | Hierarchical causal/execution graph, virtual oracles, backtracking, counterfactual attribution | LLM: Yes; Agent: analyzed MAS, offline attribution system rather than online Agent | Public benchmark; non-production Agent trajectories; no infrastructure telemetry | Medium | R0 | Batch 3; **Full Reading: Completed** |
...
- **Source / uncertainty:** Full reading completed; see [formal AIOps note](../../papers/aiops/flowfixer/notes.md). AgentFail and n8n contribute 443 annotated workflow-failure cases; the paper evaluates symbolic diagnosis, patch validation, and dynamic workflow execution, not physical remediation (Source: Sec. III–VII).
- **Full reading status:** Completed. The formal note grounds workflow candidate semantics, 16-category taxonomy, pre-execution/dynamic verification, ExperiencePool feedback, results, and the absence of rollback/human-gated network recovery.
...
- **Source / uncertainty:** Full reading completed; see [formal AIOps note](../../papers/aiops/cloud-intelligence/notes.md). This is a five-page position/framework paper with no standalone benchmark or empirical RCA/remediation evaluation; StepFly is presented as an illustrative manifestation (Source: Sec. 1–5).
- **Full reading status:** Completed. The paper supports the OKA identity/anchor/contract/lifecycle model and bounded escalation principle, but not a validated physical-network RCA or autonomous-remediation capability.
...
- **Source / uncertainty:** Full reading completed; see [formal AIOps note](../../papers/aiops/cloud-opsbench/notes.md). The paper is a five-page paradigm/prototype proposal: it does not report a completed incident-level benchmark result table, and the exact benchmark scale/release status remain unclear (Source: Sec. 1–4).
- **Full reading status:** Completed. The paper establishes the state-snapshot, mocked read-only interface, and process-centric evaluation design, but not production deployment, network RCA, remediation, or recovery evaluation.
...
- **Method family:** Adaptive in-context retrieval over prompt-summary-mitigation exemplars; an explicit Plan→Act workflow uses one LLM for summary/diagnosis/mitigation planning and another code-generation stage for Ansible scripts.
- **AIOps category:** Multimodal alert management, RCA, mitigation, and remediation.
- **Data and evidence status:** Metrics, logs, traces, and alerts are explicitly named and time/service aligned; prompt-level multimodal context. Evaluation uses MicroSS/MSDS and a Robot Shop fault-injection testbed; no live production deployment is demonstrated.
...
- **Source / uncertainty:** Full reading completed; see [formal AIOps note](../../papers/aiops/aim/notes.md), Sec. 3–4.6. The RCA signal is category/text alignment and the plan-to-code path has limited binary remediation success.
- **Full reading status:** Completed. See [formal AIOps note](../../papers/aiops/aim/notes.md). Data integration, adaptive prompting, category alignment, human evaluation, cross-dataset transfer, and plan execution are grounded in Sec. 3–4.6.

### 7. [StepFly](../../papers/aiops/stepfly/notes.md) (`stepfly`)
...
- **Method family:** Improves TSG quality, extracts execution DAGs and Query Preparation Plugins, and uses a scheduler/executor plus structured key-value working memory and parallel execution.
- **AIOps category:** Agentic incident diagnosis, troubleshooting, triage, and mitigation support.
- **Data and evidence status:** TSGs, service logs, metrics, DevOps/change data, incident history, and plugin outputs; procedural multi-source evidence rather than feature-level multimodal fusion. Study includes 92 real-world TSGs, 15 TSGs/86 query templates for preprocessing evaluation, and 80 real incidents for execution evaluation; code and synthesized incident data are public.
...
- **Source / uncertainty:** Full reading completed; see [formal AIOps note](../../papers/aiops/stepfly/notes.md), Sec. 3–7. The prototype is reported in operational use, while the controlled evaluation remains guide-dependent and mostly read-only.
- **Full reading status:** Completed. See [formal AIOps note](../../papers/aiops/stepfly/notes.md). TSG quality, DAG/QPP extraction, scheduler/executor, memory, baselines, parallelism, safety, and operational usage are grounded in Sec. 3–7.
...
- **Source / uncertainty:** Full reading completed; see [formal AIOps note](../../papers/aiops/chief/notes.md). Who&When contains 184 failed Agent trajectories with expert consensus; the method is offline Agent-trace attribution, not infrastructure AIOps (Source: Sec. 3–7).
- **Full reading status:** Completed. HCG, virtual-oracle backtracking, counterfactual attribution, single-root limitation, benchmark metrics, and ablations are grounded in the formal note.
