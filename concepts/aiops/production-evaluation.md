...
TSGen adds a separate case: its private historical corpus and deployment of a human-reviewed guide-generation aid support production-data and operational-use claims, but not autonomous diagnosis or remediation claims (Source: [TSGen note](../../papers/aiops/tsgen/notes.md), Sec. 5, Sec. 7.2). Cloud-OpsBench supplies the complementary controlled-evaluation case: immutable state and deterministic mocked tools improve reproducibility and process auditing, but its injected prototype/testbed setting is not production data, production-scale evaluation, or production deployment (Source: [Cloud-OpsBench note](../../papers/aiops/cloud-opsbench/notes.md), Sec. 2–4).
